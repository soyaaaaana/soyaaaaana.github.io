/**
 * 現在のサイト（画面）がTwitterのプロフィール画面かどうかを取得します。
 * @returns プロフィール画面かどうか（boolean）
 */
...
function nameUpdate() {
    ...
    var id = ["supu_nhk300856", "nikkorin_yuki"] // idの配列のインデックスとtextの配列のインデックスの内容はそれぞれ一緒にさせる必要がある（？）　例えば、idのインデックス1に"supu_nhk300856"があればtextのインデックス1にその@から始まるユーザー名に合う名前にする必要がある。
    var text = ["supu@すこん部🌽", "にっこりんゆき"]
    /**
     * 名前を変更します。
     * @param {object} 変更する名前の要素
     * @param {string} 名前の変更先文字列
     * @param {boolean} 要素を上書きするかどうか
     * @param {string} span要素を生成するときのclass文字列
     */
    ...
    for (let count=0;count<id.length;count++)
    {
        ...
        // アカウントのドロップダウンメニュー（ツイートで@入力した時にでてくるあれ）
...
const observer = new MutationObserver(function(){
    // 変更を検出した後にnameUpdate関数で要素を変更するのでループしないようにcheck変数でなんかやる（？）
    // check変数がfalseだったらtrueに変えてリターンする
    if (!check) {
        check = !check
        return
    }
